...
def upgrade():
    ...
    sa.Enum('PENDING', 'CANCELLED', 'FAILED', 'CHARGEDBACK', 'SUCCESS', name='paymentstatus').create(op.get_bind())
    sa.Enum('LKR', 'USD', name='paymentcurrency').create(op.get_bind())
    op.create_table('payment',
    sa.Column('merchant_id', sqlmodel.sql.sqltypes.AutoString(length=255), nullable=False),
    ...
    sa.Column('items', sqlmodel.sql.sqltypes.AutoString(length=1000), nullable=False),
    sa.Column('currency', postgresql.ENUM('LKR', 'USD', name='paymentcurrency', create_type=False), nullable=True),
    sa.Column('amount', sa.Float(), nullable=False),
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('order_id', sa.Integer(), sa.Identity(always=False), nullable=False),
    sa.Column('status', postgresql.ENUM('PENDING', 'CANCELLED', 'FAILED', 'CHARGEDBACK', 'SUCCESS', name='paymentstatus', create_type=False), nullable=True),
...
def downgrade():
    ...
    sa.Enum('PENDING', 'CANCELLED', 'FAILED', 'CHARGEDBACK', 'SUCCESS', name='paymentstatus').drop(op.get_bind())
    # ### end Alembic commands ###
