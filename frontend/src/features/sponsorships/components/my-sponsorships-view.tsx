...
import { useGetMySponsorships } from '@/features/sponsorships/api';
import { PAYMENT_STATUS } from '@/types/api';

export const MySponsorshipsView = () => {
  const [filterStatus, setFilterStatus] = useState<
    'all' | 'success' | 'pending' | 'failed'
  >('all');

  // Fetch user's sponsorships from API
  const sponsorshipsQuery = useGetMySponsorships({
    page: 1,
  });
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              LKR {totalSponsored.toFixed(2)}
            </div>
            <p className="text-xs text-muted-foreground">
              Across {successfulSponsorships} successful sponsorships
            </p>
          </CardContent>
        </Card>
                      ...
                      <div className="flex-1">
                        <div className="mb-2 flex items-center gap-2">
                          ...
                          <h3 className="font-semibold text-gray-900">
                            {sponsorship.recipient?.firstname}{' '}
                            {sponsorship.recipient?.lastname}
                          </h3>
